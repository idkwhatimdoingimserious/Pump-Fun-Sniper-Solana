...
const CUSTOM_RPC_URL = "YOUR_RPC_HERE";
...
const displayLatestCoins = async (limit = 20) => {
    ...
    const filteredCoins = await filterCoins(latestCoins).then((coins) =>
        coins.slice(0, limit),
    );
    ...
    return answers.selectedCoin === "return"
        ? null
        : filteredCoins[answers.selectedCoin];
};
...
const buyCoinByContract = async (connection, payer) => {
    ...
    savePurchasedCoins(purchasedCoins);
    console.log(
        `Monitoring price. Will sell when profit reaches ${PROFIT_PERCENTAGE}%...`,
    );
        ...
        while (true) {
            ...
            if (action === "buy_latest_coin" || action === "buy_king_coin") {
                ...
                if (action === "buy_latest_coin") {
                    const latestCoins = await getLatestCoin(40); // Fetch more coins
                ...
                console.log(
                    `Buying ${BUY_AMOUNT_SOL} SOL worth of ${coin.symbol}...`,
                );
                ...
                if (!boughtTokens) {
                    console.error(
                        `Failed to buy ${coinType} coin. Returning to menu...`,
                    );
                    continue;
                }
                ...
                while (true) {
                    ...
                    if (coinAction === "buy") {
                        console.log(
                            `Buying ${BUY_AMOUNT_SOL} SOL worth of ${selectedCoin.symbol}...`,
                        );
